...
module ActionDispatch::Routing
  class Mapper
    ...
    def wash_out(controller_name, options={})
      ...
      match "#{controller_name}/action" => WashOut::Router.new(controller_class_name, options[:default_action]), :via => [:get, :post], :defaults => { :controller => controller_class_name, :action => '_action' }, :format => false
    end
  end
end
